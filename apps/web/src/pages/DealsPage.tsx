    ...
    <AppShell>
      <div className="p-6 max-w-5xl mx-auto space-y-6">
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-0.5">Every client relationship, from brief to settlement.</p>
          </div>
      ...
      <Dialog open={!!deleteId} onOpenChange={(v) => { if (!v) setDeleteId(null); }}>
        <DialogContent>
          ...
          <p className="text-sm text-muted-foreground py-2">This will permanently delete this engagement and all associated data. Are you sure?</p>
...
function DealCard({ deal, isMenuOpen, onMenuToggle, onMenuClose, onNavigate, onDelete }: DealCardProps) {
  return (
    <div className="group relative flex items-center gap-4 rounded-lg border border-border bg-card px-4 py-3 shadow-[var(--shadow-card)] hover:border-primary/30 transition-colors">
      <button className="flex-1 text-left" onClick={onNavigate}>
        <div className="flex items-center gap-3 min-w-0">
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-muted-foreground truncate">{deal.suburb} · {deal.geoSegment}</p>
          </div>
      ...
      <div className="relative shrink-0">
        <button
          className="rounded p-1 text-muted-foreground hover:text-foreground hover:bg-[var(--surface-tinted)] transition-colors"
          onClick={(e) => { e.stopPropagation(); onMenuToggle(); }}
        >
          <MoreHorizontal size={16} />
        </button>
        {isMenuOpen && (
          <div className="absolute right-0 top-full mt-1 z-10 w-40 rounded-md border border-border bg-card shadow-[var(--shadow-elevated)]">
            <button
